...
class MemoryNetwork(object):
    def __init__(self, sentence_size, vocab_size, candidates_size,
        ...
        self._facts = tf.compat.v1.placeholder(
            tf.int32, [None, None, sentence_size], name="facts")
        self._questions = tf.compat.v1.placeholder(
            tf.int32, [None, sentence_size], name="questions")
        self._answers = tf.compat.v1.placeholder(
            tf.int32, [None], name="answers")

        # Define trainable variables used for inference
        with tf.compat.v1.variable_scope("MemoryNetwork"):
        ...
        cross_entropy_sum = tf.reduce_sum(
            input_tensor=cross_entropy, name="cross_entropy_sum")
        ...
        self.predict_op = tf.argmax(input=logits, axis=1, name="predict_op")

        # Load session and initialize all variables
        self._session = session
        self._session.run(tf.compat.v1.initialize_all_variables())

    def _input_module(self, facts):
        with tf.compat.v1.variable_scope("InputModule"):
            facts_emb = tf.nn.embedding_lookup(params=self.word_emb_matrix,
                                               ids=facts)
            return tf.reduce_sum(input_tensor=facts_emb, axis=2)

    def _question_module(self, questions):
        with tf.compat.v1.variable_scope("QuestionModule"):
            questions_emb = tf.nn.embedding_lookup(
                params=self.word_emb_matrix, ids=questions)
            return tf.reduce_sum(input_tensor=questions_emb, axis=1)

    def _memory_module(self, questions_emb, facts_emb):
        with tf.compat.v1.variable_scope("MemoryModule"):
            ...
            for hop in range(self._hops):
                ...
                context_temp = tf.transpose(
                    a=tf.expand_dims(context_vectors[-1], -1), perm=[0, 2, 1])
                similarity_scores = tf.reduce_sum(
                    input_tensor=facts_emb * context_temp, axis=2)
                # Calculate similarity probabilities
                probs = tf.nn.softmax(similarity_scores)
                # Perform attention multiplication
                probs_temp = tf.transpose(a=tf.expand_dims(probs, -1),
                                          perm=[0, 2, 1])
                facts_temp = tf.transpose(a=facts_emb, perm=[0, 2, 1])
                context_rep = tf.reduce_sum(input_tensor=facts_temp * probs_temp, axis=2)
    ...
    def _output_module(self, context_vector):
        with tf.compat.v1.variable_scope("OuptutModule"):
            candidates_emb = tf.nn.embedding_lookup(params=self.output_word_emb_matrix,
                                                    ids=self._candidates_vec)
            candidates_emb_sum = tf.reduce_sum(input_tensor=candidates_emb, axis=1)
            return tf.matmul(context_vector, tf.transpose(a=candidates_emb_sum))
